...
ggplot(bp, aes(x=diastolic_index_bpm, y=diastolic_gg_0604)) +
  ...
  labs(title="Diastolic Blood Pressure, mm Hg",
       x ="Garmin Index BPM",
       y = "GG 0604") +
  coord_fixed(ratio = 1, xlim = c(60, 90), ylim = c(60, 90)) +
  geom_abline(linetype = 2, color = "red") +
  geom_smooth(method=lm)
...
ggplot(bp, aes(x=hr_index_bpm, y=hr_gg_0604)) +
  ...
  labs(title="Heart Rate, bpm",
       x ="Garmin Index BPM",
       y = "GG 0604") +
  coord_fixed(ratio = 1, xlim = c(50, 70), ylim = c(50, 70)) +
  geom_abline(linetype = 2, color = "red") +
  geom_smooth(method=lm)

```
## Deming regression and Bland-Altman plots:
  
```{r deming.regression, echo=FALSE}

#--- Deming regression and Blant-Altman Plots
...
table.1 <- kable(table.1, digits = 2, caption = "Blood Pressure Accuracy Metrics" )
table.1
